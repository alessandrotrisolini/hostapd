...
static void ieee802_1x_delete_receive_sa(struct ieee802_1x_kay *kay, struct receive_sa *sa)
{
...
/**
 * ieee802_1x_kay_deinit_data_key - release reference on a key
...
static void ieee802_1x_delete_transmit_sa(struct ieee802_1x_kay *kay, struct transmit_sa *sa)
{
			      ...
			      struct ieee802_1x_mka_ki *lki)
{
	...
	while ((txsa = lookup_txsa_by_an(principal->txsc, latest_sak->an)) != NULL)
		ieee802_1x_delete_transmit_sa(kay, txsa);
			  ...
			  enum mka_created_mode mode, Boolean is_authenticator)
{
	...
	if (cak->len != mka_alg_tbl[kay->mka_algindex].cak_len) {
		wpa_printf(MSG_ERROR, "KaY: CAK length not follow key schema %lu", cak->len);
		return NULL;
	}
