			    ...
			    u8 type, const u8 *data, size_t datalen)
{
    ...
    */
     ...
     *
     */
	if (data[10] == 0x16 && data[15] == 0x02) {
		os_memset(sta->server_random, 0, 32);
		os_memcpy(sta->server_random, &data[21], 32);
		wpa_hexdump(MSG_ERROR, "SERVER RANDOM", sta->server_random, 32);
	}
#endif /* CONFIG_MACSEC */
...
#ifdef CONFIG_MACSEC
    /*
     * Check for TLS Handshake and get Client Random
     *
     */
	if (type == 0x0D && data[2] == 0x16 && data[7] == 0x01) {
		os_memset(sta->client_random, 0, 32);
		os_memcpy(sta->client_random, &data[13], 32);
		wpa_hexdump(MSG_ERROR, "CLIENT RANDOM", sta->client_random, 32);
	}	
#endif /* CONFIG_MACSEC */
			...
			size_t len)
{
	...
	case IEEE802_1X_TYPE_EAPOL_MKA:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
				HOSTAPD_LEVEL_DEBUG, "received EAPOL-MKA "
				"from STA");
		break;
#endif /* CONFIG_MACSEC */
	default:
				...
				int remediation)
{
...
#ifdef CONFIG_MACSEC
	/*
	 * u8 *cak - Connectivity Association Key (CAK) 
	 * u8 *ckn - Connectivity Association Key Name (CKN)
	 * u8 *sid - EAP-TLS Session-Id
	 */	
	u8 *cak;
	u8 *ckn;
	u8 *sid;
	...
	if(success) {
		 ...
		 * Computing CAK
		 */
		ieee802_1x_cak_128bits_aes_cmac(key, hapd->own_addr, 
						sta->addr, cak);

		wpa_snprintf_hex(cak_str, 33, cak, 16);
		wpa_hexdump(MSG_ERROR, "CAK", cak, 16);

		/*
		 * Session-Id = 0x0D || client_random || server_random
		 * (see RFC 5216)
		 */
		
		sid[0] = 13;
		os_memcpy(sid + 1, sta->client_random, 32);
		os_memcpy(sid + 33, sta->server_random, 32);

		/*
		 * Computing CKN
		 */
		ieee802_1x_ckn_128bits_aes_cmac(key, hapd->own_addr,
						sta->addr, sid, 65, 
						ckn);

		wpa_hexdump(MSG_ERROR, "CKN", ckn, 16);
		wpa_snprintf_hex(ckn_str, 33, ckn, 16);
		wpa_hexdump(MSG_ERROR, "Session Id", sid, 65);	
		wpa_printf(MSG_DEBUG, "Creating MKA for:" MACSTR, 
				MAC2STR(sta->addr));

		/*
		 * Create MKA session with CAK and CKN
		 */
		if(ieee802_1x_create_hapd_mka(hapd, cak, ckn) == NULL){
			wpa_printf(MSG_ERROR, "MKA alloc failed");
			ieee802_1x_dealloc_hapd_mka(hapd);
		}
