...
static int macsec_qca_lookup_channel(struct channel_map *map, struct ieee802_1x_mka_sci *sci, u32 *channel)
{
	int i;

	for (i = 0; i < MAXSC; i++) {
		if (os_memcmp(&map[i].sci, sci, sizeof(struct ieee802_1x_mka_sci))) {
			*channel = i;
			return 0;
		}
	}
...
static void macsec_qca_register_channel(struct channel_map *map, struct ieee802_1x_mka_sci *sci, u32 channel)
{
	os_memcpy(&map[channel].sci, sci, sizeof(struct ieee802_1x_mka_sci));
}


static int macsec_qca_lookup_receive_channel(struct macsec_qca_data *drv, struct receive_sc *sc, u32 *channel)
{
	return macsec_qca_lookup_channel(drv->receive_channel_map, &sc->sci, channel);
}

static void macsec_qca_register_receive_channel(struct macsec_qca_data *drv, struct receive_sc *sc, u32 channel)
{
	macsec_qca_register_channel(drv->receive_channel_map, &sc->sci, channel);
}


static int macsec_qca_lookup_transmit_channel(struct macsec_qca_data *drv, struct transmit_sc *sc, u32 *channel)
{
	return macsec_qca_lookup_channel(drv->transmit_channel_map, &sc->sci, channel);
}

static void macsec_qca_register_transmit_channel(struct macsec_qca_data *drv, struct transmit_sc *sc, u32 channel)
{
	macsec_qca_register_channel(drv->transmit_channel_map, &sc->sci, channel);
}
...
static int macsec_qca_create_receive_sa(void *priv, struct receive_sa *sa)
{
	...
	ret += nss_macsec_secy_rx_sak_set(drv->secy_id, channel, sa->an, &rx_sak);
...
static int macsec_qca_enable_receive_sa(void *priv, struct receive_sa *sa)
{
	...
	wpa_printf(MSG_DEBUG, "%s: channel=%d, an=%d", __func__, channel, sa->an);

	ret += nss_macsec_secy_rx_sa_en_set(drv->secy_id, channel, sa->an, TRUE);
...
static int macsec_qca_disable_receive_sa(void *priv, struct receive_sa *sa)
{
	...
	ret += nss_macsec_secy_rx_sa_en_set(drv->secy_id, channel, sa->an, FALSE);
...
static int macsec_qca_create_transmit_sa(void *priv, struct transmit_sa *sa)
{
	...
	ret += nss_macsec_secy_tx_sa_next_pn_set(drv->secy_id, channel, sa->an,
						 sa->next_pn);
	ret += nss_macsec_secy_tx_sak_set(drv->secy_id, channel, sa->an, &tx_sak);
...
static int macsec_qca_enable_transmit_sa(void *priv, struct transmit_sa *sa)
{
	...
	ret += nss_macsec_secy_tx_sa_en_set(drv->secy_id, channel, sa->an, TRUE);
...
static int macsec_qca_disable_transmit_sa(void *priv, struct transmit_sa *sa)
{
	...
	ret += nss_macsec_secy_tx_sa_en_set(drv->secy_id, channel, sa->an, FALSE);
