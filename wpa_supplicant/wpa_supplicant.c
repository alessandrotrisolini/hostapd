...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	...
	if ((!wpa_s->p2p_mgmt ||
		...
		if(!wpa_s->ovs_port_ifname[0]) {
			wpa_s->l2 = l2_packet_init(wpa_s->ifname,
					   	NULL,
					   	ETH_P_EAPOL,
					   	wpa_supplicant_rx_eapol, wpa_s, 0);
		} else {
			addr = os_zalloc(ETH_ALEN);

			if(l2_packet_get_mac_addr(wpa_s->ovs_port_ifname, addr) < 0) {
				os_free(addr);
				return -1;
			}
				     ...
				     struct wpa_interface *iface)
{
	...
	if(iface->ovs_port_ifname) {
		os_strlcpy(wpa_s->ovs_port_ifname, iface->ovs_port_ifname,
				sizeof(wpa_s->ovs_port_ifname));
		wpa_printf(MSG_DEBUG, "OVS port name: %s",
				wpa_s->ovs_port_ifname);
	}
