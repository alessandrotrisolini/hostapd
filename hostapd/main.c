...
static int hostapd_driver_init(struct hostapd_iface *iface)
{
	...
	wpa_printf(MSG_ERROR, "Interface name: %s", hapd->conf->iface); 
		       ...
		       const char *config_fname, int debug)
{
	...
	if (!iface){
		wpa_printf(MSG_ERROR, "iface NULL");	
		return NULL;
	}
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		c = getopt(argc, argv, "b:Bde:f:hi:KP:STtu:vg:G:z:");
		if (c < 0)
			break;
		switch (c) {
		case 'h':
	...
	for (i = 0; i < interfaces.count; i++) {
		...
		wpa_printf(MSG_DEBUG, "Interface name : %s", if_name);
		...
		if (ovs_br_name!=NULL) {
			interfaces.iface[i]->bss[0]->ovs_br_name = os_strdup(ovs_br_name);
			os_free(ovs_br_name);
		}
	}
